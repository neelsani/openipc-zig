...
var PThread = {
  ...
  allocateUnusedWorker() {
    ...
    worker = new Worker(new URL("index.js", import.meta.url), {
      "type": "module",
      // This is the way that we signal to the Web Worker that it is hosting
      // a pthread.
      "name": "em-pthread-" + PThread.nextWorkerID
...
function ___syscall_pipe(fdPtr) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(4, 0, 1, fdPtr);
  try {
    if (fdPtr == 0) {
...
function ___syscall_poll(fds, nfds, timeout) {
  if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(5, 0, 1, fds, nfds, timeout);
  try {
    var nonzero = 0;
...
var readLatin1String = ptr => {
  var ret = "";
  var c = ptr;
  while ((growMemViews(), HEAPU8)[c]) {
    ret += embind_charCodes[(growMemViews(), HEAPU8)[c++]];
  }
  return ret;
};
...
var stringToUTF32 = (str, outPtr, maxBytesToWrite) => {
  ...
  for (var i = 0; i < str.length; ++i) {
    ...
    (growMemViews(), HEAP32)[((outPtr) >> 2)] = codeUnit;
    outPtr += 4;
...
var __emval_coro_make_promise = (resolveHandlePtr, rejectHandlePtr) => Emval.toHandle(new Promise((resolve, reject) => {
  ...
  (growMemViews(), HEAPU32)[((resolveHandlePtr) >> 2)] = Emval.toHandle(resolve);
  (growMemViews(), HEAPU32)[((rejectHandlePtr) >> 2)] = Emval.toHandle(rejectWithCurrentException);
}));
...
__emval_coro_suspend.isAsync = true;
...
function _js_freeKeyBuffer(buffer) {
  if (buffer) {
    _free(buffer);
  }
}

function _js_getKeyBuffer(lengthPtr) {
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, pthreadCreateProxied, ___syscall_fcntl64, ___syscall_pipe, ___syscall_poll, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write ];
...
function __asyncjs__em_libusb_wait_async(ptr, expected_value, timeout) {
  return Asyncify.handleAsync(async () => {
    await Atomics.waitAsync((growMemViews(), HEAP32), ptr >> 2, expected_value, timeout).value;
  });
}
...
function usbi_em_copy_from_dataview(dst, src) {
  ...
  (growMemViews(), HEAPU8).set(src, dst);
}
...
function assignWasmImports() {
  wasmImports = {
    ...
    /** @export */ __asyncjs__em_libusb_wait_async,
    ...
    /** @export */ _emval_coro_make_promise: __emval_coro_make_promise,
    /** @export */ _emval_coro_suspend: __emval_coro_suspend,
    ...
    /** @export */ _emval_iter_begin: __emval_iter_begin,
    /** @export */ _emval_iter_next: __emval_iter_next,
    ...
    /** @export */ displayFrame: _displayFrame,
...
var dynCall_iiijiji = Module["dynCall_iiijiji"] = createExportWrapper("dynCall_iiijiji", 7);
