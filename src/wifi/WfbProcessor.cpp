      ...
      count_p_fec_recovered(0), count_p_lost(0), count_p_bad(0), count_p_override(0), dcb(cb)
{
    ...
    static const uint8_t hardcoded_keypair[] = {
    ...
    // In Emscripten, 'keypair' is actually the binary content of the keyfile

       // The first part is rx_secretkey
    memcpy(rx_secretkey, hardcoded_keypair, crypto_box_SECRETKEYBYTES);
    // The second part is tx_publickey
    memcpy(tx_publickey, hardcoded_keypair + crypto_box_SECRETKEYBYTES, crypto_box_PUBLICKEYBYTES);
#else
    // Original file reading code for native environments
    FILE *fp;
